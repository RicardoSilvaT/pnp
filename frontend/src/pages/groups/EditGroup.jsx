// src/pages/groups/EditGroup.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { ArrowLeft, Shield, Lock, Check, X, Loader } from 'lucide-react';
import { useGroup, useGroups, usePermissions } from '../../hooks/useGroups';
import Button from '../../Components/common/Button';

const EditGroup = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { group, loading: groupLoading, error } = useGroup(id);
  ...
  useEffect(() => {
    if (group) {
      setFormData({
        name: group.name || '',
        permissions: group.permissions?.map(p => p.id || p) || []
      });
    }
  }, [group]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-green-50 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-green-800 font-semibold text-lg">Cargando grupo...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-red-50 border-2 border-red-200 rounded-xl p-6 shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-red-800 font-semibold text-lg">Error al cargar grupo</h3>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-yellow-800 font-semibold text-lg">Grupo no encontrado</h3>
                <p className="text-yellow-600">El grupo solicitado no existe.</p>
              </div>
            </div>
            <Button
              variant="outline"
          ...
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm md:text-base">
                Modifica la información de {group.name}
              </p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="bg-white rounded-2xl shadow-xl border border-green-100 overflow-hidden">
              ...
              <div className="p-6 md:p-8">
                {/* Group Name */}
                <div className="mb-8">
                  <label htmlFor="edit-group-name" className="block text-sm font-bold text-gray-700 mb-3">
                    Nombre del Grupo *
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder="Ej: Administradores, Editores, Visualizadores"
                  />
                  {errors.name && (
                ...
                <div>
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                    <span className="block text-sm font-bold text-gray-700 mb-3 md:mb-0">
                      Permisos del Grupo
                    </span>
                    <div className="flex items-center space-x-4">
                      <Button
                  ...
                  {/* Search Permissions */}
                  <div className="mb-6">
                    <label htmlFor="edit-search-permissions" className="sr-only">
                      Buscar permisos
                    </label>
                    <input
                      type="text"
                      id="edit-search-permissions"
                              ...
                              {modelPermissions.map(permission => (
                                ...
                                >
                                  ...
                                  <div className="ml-3 flex-1">
                                    ...
                                    <p id={`edit-permission-${permission.id}-desc`} className="text-xs text-gray-500">
                                      {permission.codename}
                                    </p>
                                  </div>
                                </label>
